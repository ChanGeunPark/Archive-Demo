...
import { createDemoAiResponse, splitForStream } from "@/lib/ai-chat-demo/generator";
import {
  getDemoCharacter,
  getDemoChatHistory,
  saveDemoMessage,
...
export async function POST(request: Request) {
  ...
  const aiResponse = createDemoAiResponse({
    character,
    message: userMessage,
    history,
  });
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      for (const chunk of splitForStream(aiResponse)) {

...
const serviceComparison = [
  ...
  },
  {
    ...
    demo: "loading.tsx + 서버 fetch 1회 + initialWork 즉시 표시 (백그라운드 revalidate)",
  },
  {
    item: "인증 · 거래",
...
const designDecisions = [
  {
    ...
    reason:
      "첫 렌더부터 컬럼 너비에 맞춰 카드 높이를 예약해, 이미지 로드 전후로 레이아웃이 흔들리는 문제를 줄였습니다.",
    result:
      "가로형 이미지 오버레이 기준은 DOM 측정값 대신 width/height 메타데이터로 판단합니다.",
  },
  {
    title: "Apollo cache merge",
    reason:
      "상세에서 뒤로 돌아왔을 때 이미 불러온 페이지와 스크롤 위치를 즉시 복원할 수 있습니다.",
    result:
      "필터 조합마다 별도 캐시 엔트리가 생기므로 `keyArgs`를 명확히 설계해야 합니다.",
  },
  {
    title: "Realtime = 트리거",
    reason:
      "이벤트 payload를 화면 상태로 직접 쓰지 않아, 서버 기준의 거래 정합성을 유지합니다.",
    result:
      "변경마다 GraphQL refetch가 필요해, payload 직접 반영보다 네트워크 호출이 한 번 더 발생합니다.",
  },
] as const;
...
const demoSeedCache = `// WorkMain — initialWork fallback + Apollo cache seed
useLayoutEffect(() => {
...
useQuery(WORK_DETAIL_QUERY, {
  fetchPolicy: "cache-and-network", // initialWork 즉시 표시 + 백그라운드 revalidate
});`;
  ...
  return (
    <section className="space-y-12">
          ...
          <ul className="space-y-3">
            ...
            <li>
              ...
              >
                <Typography
                  ...
                  color="#17191C"
                >
                  작품 상세 — initialWork + cache-and-network:{" "}
                </Typography>
                RSC에서 받은{" "}
                <Typography
                  as="span"
                ...
                에서 Apollo 캐시를 seed한 뒤 백그라운드 revalidate합니다.
                구매·제안 등 mutation은{" "}
                <Typography
                  as="span"
                  variant="body3"
            ...
            <li>
              ...
              >
                ...
                RSC가 준비되는 동안 Discover가 멈춘 것처럼 보이지 않도록, 라우트
                전환 직후 detail 스켈레톤을 먼저 표시합니다.
              </Typography>
            </li>
            <li>
              <Typography
      ...
      <div className="space-y-5">
        <SectionHeading
          title="작품 상세 진입 로딩"
          description="카드 클릭 후 전환 피드백·서버 fetch dedupe·initialWork 즉시 표시를 조합해 체감 지연을 줄였습니다."
        />
        <div className="rounded-2xl border border-[#EDEEEF] bg-white p-4 shadow-[0_8px_30px_rgba(15,23,42,0.04)] sm:p-5">
          <Typography
            variant="body3"
          ...
          >
            ...
            로 라우트 전환 직후 detail 스켈레톤을 먼저 보여주고,{" "}
            <Typography as="span" variant="body3" weight={600} color="#17191C">
            ...
            와 page가 같은 요청 안에서 Supabase를 1회만 조회합니다. RSC가 끝나면{" "}
            <Typography as="span" variant="body3" weight={600} color="#17191C">
              (3) initialWork
            </Typography>
            로{" "}
            ...
            이 상세 본문을 바로 렌더하고,{" "}
            <Typography as="span" variant="body3" weight={600} color="#17191C">
              (4) seedWorkDetailCache + cache-and-network
            </Typography>
            로 Apollo 캐시를 맞춘 뒤 백그라운드 revalidate합니다. hover
            prefetch는 불필요한 GraphQL 호출을 늘릴 수 있어 사용하지 않습니다.
          </Typography>
        </div>
        <div className="grid gap-4">
          <CodeBlock
      ...
      <div className="space-y-5">
        ...
        <div className="rounded-2xl border border-[#EDEEEF] bg-white p-4 shadow-[0_8px_30px_rgba(15,23,42,0.04)] sm:p-5">
          <Typography
            ...
            className="break-keep leading-relaxed"
          >
            다음 페이지를 어디서부터 가져올지 서버에 알려주는 커서는 마지막
            작품의{" "}
            <Typography as="span" variant="body3" weight={600} color="#17191C">
              createdAt
            </Typography>
            ...
            로 인코딩합니다. 등록 시각이 같은 작품이 여러 개일 수 있어{" "}
            <Typography as="span" variant="body3" weight={600} color="#17191C">
              created_at DESC, id DESC
            </Typography>
            정렬에서도 페이지 경계가 어긋나지 않게 하기 위함입니다. 불러온
            페이지와{" "}
            <Typography as="span" variant="body3" weight={600} color="#17191C">
              endCursor
            </Typography>
            는 Apollo 캐시에 남으므로, 상세에서 돌아올 때 sessionStorage에
            커서를 따로 저장하지 않습니다.
          </Typography>
          <Typography
            variant="body3"
            ...
            className="mt-3 break-keep leading-relaxed"
          >
            반면 스크롤 위치는 캐시에 없으므로, 작품 상세로 떠날 때{" "}
            <Typography as="span" variant="body3" weight={600} color="#17191C">
              scrollY
            </Typography>
            만 sessionStorage에 저장합니다. Discover로 돌아오면{" "}
            <Typography as="span" variant="body3" weight={600} color="#17191C">
              cache-first
            </Typography>
            로 이미 불러 둔 목록을 먼저 보여 준 뒤, 저장된 위치까지 스크롤할
            높이가 부족하면{" "}
            <Typography as="span" variant="body3" weight={600} color="#17191C">
              fetchMore
            </Typography>
            를 반복해 페이지를 채웁니다. 필터를 바꾸면 목록이 짧아져 sentinel이
            화면에 바로 보일 수 있는데, 이때는 사용자가 아직 스크롤하지
            않았으므로 다음 페이지를 자동으로 부르지 않습니다. 실제 스크롤이
            일어난 뒤에만 무한 스크롤이 풀리도록{" "}
            <Typography as="span" variant="body3" weight={600} color="#17191C">
              canLoadMoreRef
            </Typography>
            게이트를 둡니다.
          ...
          <ul className="space-y-3">
            ...
            <li>
              ...
              >
                ...
                loading.tsx로 전환 피드백을 주고, React.cache·initialWork·Apollo
                seed로 RSC 완료 후 상세 본문을 바로 표시합니다.
              </Typography>
            </li>
            <li>
              <Typography
